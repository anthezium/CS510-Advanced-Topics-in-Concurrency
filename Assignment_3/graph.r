...
gen_graph <- function(data, xc, yc, f, name, n_breaks, lbs){
  ...
  if(nrow(data) == 0) {
    print(paste0("No data for ",name,", not generating a graph."))
    return ()
  }
...
gen_graph(subset(ticks_by_test_and_nt, critical_section_accesses == 0), "nt", "ticks", is_graphable, paste0(name, "_ticks_00accesses"), n_breaks, 
         labs(title = "Test Duration vs Number of Threads (empty critical section)",
              x     = "Number of Threads",
              y     = "Test Duration"))

gen_graph(subset(ticks_by_test_and_nt, critical_section_accesses == 8), "nt", "ticks", is_graphable, paste0(name, "_ticks_08accesses"), n_breaks, 
